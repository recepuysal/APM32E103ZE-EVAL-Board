...
/* Ported from Geehy's bsp_sdio.c (GeehySemi/APM32E10x_EVAL_SDK) - a
 * self-contained SDIO+SD-card driver reused almost verbatim (pin map, SDIO
 * command sequencing, DMA transfer). Adapted: header renamed, APM_EVAL_DelayMs
 * replaced with this project's own tick-based delay (below), and the two
 * printf-based single/multi-block test demo functions were dropped (not part
 * of the driver API used by diskio.c). */
extern volatile uint32_t g_tickMs;

static void SD_DelayMs(uint32_t ms)
{
...
void SD_ShowCardInfo(void)
{
    ...
    /** Card capacity */
    printf("Card Capacity:%lu MB\r\n",(unsigned long)(SDCardInfo.CardCapacity>>20));
...
/* Per SD_Process_Isr()'s own doc comment: this belongs in the SDIO IRQ vector. */
